...
// Make sure all functions are in global scope (no wrapping)
...
async function fetchProductsFromScript() {
    try {
        ...
        if (result.status === 'success' && result.data && result.data.length > 1) {
            ...
            console.log(`Loaded ${window.ECOM_STATE.products.length} products from Google Apps Script`);
...
function showNoProductsMessage() {
    ...
    grid.innerHTML = `
        <div style="grid-column: 1/-1; text-align: center; padding: 60px 20px; color: #6c757d; font-size: 18px;">
            ...
            <div style="font-size: 14px; margin-top: 8px;">Add products to your Google Sheet with columns: Serial No, Name, Price, Category, Image</div>
...
console.log('✓ db-integration.js loaded - Functions available:', {
    isValidURL: typeof isValidURL !== 'undefined',
    convertGoogleDriveUrl: typeof convertGoogleDriveUrl !== 'undefined',
    fetchProductsFromScript: typeof fetchProductsFromScript !== 'undefined',
    refreshProducts: typeof refreshProducts !== 'undefined',
    showLoadingMessage: typeof showLoadingMessage !== 'undefined',
    showErrorMessage: typeof showErrorMessage !== 'undefined',
    showNoProductsMessage: typeof showNoProductsMessage !== 'undefined'
});
